...
from dataloader.dataset import MRI_dataset
...
def train(config, train_dataset, model):
    ...
    for epoch in range(int(config.num_train_epochs)):
        for step, batch in enumerate(train_dataloader):
            ...
            input_images = input_images.to(torch.device(config.device)).squeeze(0).unsqueeze(1)
            input_texts = input_texts.to(torch.device(config.device)).squeeze(0)
...
def main():
    ...
    logger = setup_logger("MRI_TRAIN", config.logs, 0, filename = "training_logs.txt")
    ...
    train_dataset = MRI_dataset(config, tokenizer)
